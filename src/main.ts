...
(async function main() {
  ...
  let num2 = (await zkapp.secret.get())!;
  console.log('current value of num is', num.toString());
  console.log('current value of num2 is', num2.toString());
  // ----------------------------------------------------

  await makeAndSendTransaction({
    feePayerPrivateKey: deployerPrivateKey,
    zkAppPublicKey: zkAppPublicKey,
    mutateZkApp: () => zkapp.update(Field(3),Field(1337)),
    transactionFee: transactionFee,
    getState: () => zkapp.num.get(),
  ...
  console.log('updated state!', zkapp.num.get().toString());
  console.log('updated state!', zkapp.secret.get().toString());

  // ----------------------------------------------------
  await makeAndSendTransaction({
    feePayerPrivateKey: deployerPrivateKey,
    zkAppPublicKey: zkAppPublicKey,
    mutateZkApp: () => zkapp.verify(Field(3),Field(1337)),
    transactionFee: transactionFee,
    getState: () => zkapp.num.get(),
  ...
  let num3 = (await zkapp.num.get())!;
  console.log('current value of num is', num3.toString());
  // ----------------------------------------------------
